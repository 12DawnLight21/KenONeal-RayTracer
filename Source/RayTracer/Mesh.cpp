...
bool Mesh::Hit(const ray_t& ray, float minDistance, float maxDistance, raycastHit_t& raycastHit)
{
	...
	for (int i = 0; i < m_vertices.size(); i += 3) //<i = 0, while less than number of vertices and increment by 3 each iteration>
	{
		Triangle triangle(m_vertices[i], m_vertices[i + 1], m_vertices[i + 2], m_material); //< set the three vertices for the triangle(m_vertices[i], [i + 1], [i + 2] >
